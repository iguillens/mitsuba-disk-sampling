...
void Scene::sampleEmitterEquiangular(const Ray &ray, PositionSamplingRecord &pRec,
		const Medium *medium, int &interactions, const Float sample) const {
	Float u = sample;

	/* Randomly pick an emitter */
	Float emPdf;
	size_t index = m_emitterPDF.sampleReuse(u, emPdf);
	const Emitter *emitter = m_emitters[index].get();

	/* Get the center of the emitter */
	emitter->samplePosition(pRec, Point2(0.5f, 0.5f));

	/* Reference frame */
	Float s = dot((pRec.p - ray.o), ray.d);
	Float D = (pRec.p - ray(s)).length();

	/* Avoid pdf singularity at light center */
	D = std::max(0.5f, std::abs(D));

	/* Sample position along the ray */
	Float a = ray.mint - s;
	Float b = ray.maxt - s;

	Float theta_a = std::atan2(a, D);
	Float theta_b = std::atan2(b, D);

	Float t = D * std::tan((1.f - u)*theta_a + u*theta_b);
	Float tpdf = D/((theta_b - theta_a)*(D*D + t*t));
	...
	pRec = PositionSamplingRecord(ray.time);
	pRec.p = ray(s + t);
	pRec.n = Normal(0, 0, 0);
	pRec.pdf = emPdf * tpdf;
...
void Scene::sampleEmitterEquiangularNonCenter(const Ray &ray,
			PositionSamplingRecord &pRec, const Medium *medium, int &interactions,
			const Point2 &pos, const Float sample) const {
	Float u = sample;
	...
	/* Get the center of the emitter */
	emitter->samplePosition(pRec, pos);
