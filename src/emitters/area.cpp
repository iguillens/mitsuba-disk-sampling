...
public:
	AreaLight(const Properties &props) : Emitter(props) {
		...
		m_double_sided = props.getBoolean("doubleSided", false);
	}
	...
	AreaLight(Stream *stream, InstanceManager *manager)
		...
		m_double_sided = (bool)stream->readInt();
	...
	void serialize(Stream *stream, InstanceManager *manager) const {
		...
		stream->writeInt((int)m_double_sided);
	}
	...
	Spectrum eval(const Intersection &its, const Vector &d) const {
		if (!m_double_sided && dot(its.shFrame.n, d) <= 0)
	...
	Spectrum evalDirection(const DirectionSamplingRecord &dRec,
		...
		if (dRec.measure != ESolidAngle || (!m_double_sided && dp < 0))
			dp = 0.0f;
	...
	Spectrum sampleDirect(DirectSamplingRecord &dRec,
		...
		if (dot(dRec.d, dRec.refN) >= 0 && (m_double_sided || (dot(dRec.d, dRec.n) < 0))
			&& dRec.pdf != 0) {
...
public:
	...
	void generateCode(std::ostringstream &oss, const std::string &evalName,
	    ...
	    if (m_double_sided) {
	    	oss << "    if (cosTheta(wo) < 0.0)" << endl
	    	    << "    	return vec3(-inv_pi);" << endl;
		} else {
			oss << "    if (cosTheta(wo) < 0.0)" << endl
			    << "    	return vec3(0.0);" << endl;
		}
		oss	<< "    return vec3(inv_pi);" << endl
			<< "}" << endl;
	}
...
Shader *AreaLight::createShader(Renderer *renderer) const {
	return new AreaLightShader(renderer, m_radiance, m_double_sided);
}
